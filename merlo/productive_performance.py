...
FROZEN_WORKLOADS: tuple[FrozenWorkload, ...] = (
    FrozenWorkload(
        "ndjson",
        "benchmarks/productive_performance/ndjson/workload.ndjson",
        548_337,
    ...
    FrozenWorkload(
        "csv",
        "benchmarks/productive_performance/csv/workload.csv",
        182_104,
    ...
    FrozenWorkload(
        "grep",
        "benchmarks/productive_performance/grep/workload.txt",
        286_389,
...
def _arm_artifact(root: Path, application: str, arm: str) -> list[str]:
    if arm == "concise_merlo":
        base = root / "merlo" / "programs" / f"productive_{application}" / "app"
        return sorted(str(path.relative_to(root)) for path in base.glob("*.mlo") if path.is_file())
    if arm == "canonical_merlo":
        base = root / "merlo" / "programs" / f"productive_{application}" / "canonical"
        return sorted(str(path.relative_to(root)) for path in base.glob("*.mlo") if path.is_file())
    if arm == "python":
        path = root / "benchmarks" / "productive_simplicity" / application / "reference.py"
        return [str(path.relative_to(root))] if path.is_file() else []
    if arm == "native":
        source = root / "benchmarks" / "productive_simplicity" / application / "reference.c"
        base = root / "benchmarks" / "productive_performance" / application
