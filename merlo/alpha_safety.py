...
from .productive_safety import run_productive_safety, validate_productive_safety_report
...
def run_alpha_safety(
    ...
    corpus = load_alpha_corpus(Path(root) / "benchmarks" / "merlo_alpha_corpus.json") if (Path(root) / "benchmarks" / "merlo_alpha_corpus.json").exists() else generate_alpha_corpus()
