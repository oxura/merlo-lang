...
from .concise_application import (
...
from .concise_precedence import (
    parse_expression,
...
_FROZEN_PREDECESSORS = {
    "merlo/programs/general_json.mlo":
        "0b696f9a6653ea5fa20124d239db37fe6853ff798abe0cbcdcb703dd9c66ff04",
    "benchmarks/merlo_general_representation_core.json":
        "63223657697b8251b63f25f952f79b0ba369a1a92abfff887dfe5c518959f7e6",
    "benchmarks/merlo_general_representation_benchmark.json":
...
def _copy_application(root: Path, destination: Path) -> Path:
    source = root / "merlo" / "programs" / "concise_json"
    for relative in (
        Path("app/json.mlo"),
        Path("app/stats.mlo"),
        Path("app/main.mlo"),
...
def _performance(root: Path, compilation: Any, artifact: Path) -> dict[str, Any]:
    ...
    concise_build = compile_project(
        root / "merlo/programs/concise_json/app/main.mlo",
        emit_native=True,
...
def run_concise_application_milestone(
    root: str | Path = ".",
    *,
    artifact_dir: str | Path = "benchmarks/concise_application_alpha",
    report_path: str | Path = "benchmarks/merlo_concise_application_alpha.json",
) -> dict[str, Any]:
    started = time.perf_counter()
    root_path = Path(root).resolve()
    artifact = root_path / artifact_dir
    artifact.mkdir(parents=True, exist_ok=True)
    entry = root_path / "merlo/programs/concise_json/app/main.mlo"
