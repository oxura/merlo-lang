...
from .native_c_backend import CEmitter, compile_c_source, compiler_version, find_c_compiler
...
def _validate_performance_cache(root: Path, primary_optimized: PerformanceMIR) -> dict[str, Any]:
    artifact_path = root / "benchmarks" / "meldra_bytes_experiment.json"
    ...
    schema_paths = (
        root / "merlo" / "performance_mir_schema_v1.json",
        root / "merlo" / "bytes_contract.py",
        root / "merlo" / "native_hir.py",
    )
    schema_digest = _canonical_digest(
        {str(path.relative_to(root)): _digest_bytes(path.read_bytes()) for path in schema_paths}
    )
    optimizer_digest = _digest_bytes((root / "merlo" / "performance_opt.py").read_bytes())
...
def collect_bytes_closure_evidence(
    root: str | Path = ".",
    *,
    output_dir: str | Path = "benchmarks/meldra_bytes_evidence_closure",
) -> dict[str, Any]:
    ...
    preregistration = root_path / "benchmarks" / "meldra_bytes_evidence_preregistered.json"
    ...
    return {
        ...
        "self_skeptical_audit": {
            "path": "benchmarks/meldra_bytes_self_skeptical_audit.json",
            "sha256": _digest_bytes((root_path / "benchmarks" / "meldra_bytes_self_skeptical_audit.json").read_bytes()),
        },
...
def finalize_bytes_closure_report(
    ...
    *,
    ...
    report_path: str | Path = "benchmarks/meldra_bytes_evidence_closure.json",
) -> dict[str, Any]:
