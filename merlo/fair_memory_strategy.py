...
from .codegen_evidence import _assembly_metrics, _compile_assembly
...
from .stage06p_benchmark import BENCHMARK_SEED, _cpu_state, _distribution, _run_one
...
def _freeze_manifest(root: Path) -> dict[str, Any]:
    ...
    prior_path = Path("benchmarks/meldra_stage06p_memory.json")
    prior = json.loads(prior_path.read_text(encoding="utf-8"))
    by_name = {item["name"]: item for item in prior["arms"]}
    frozen_generated = {
        strategy: Path(
            f"benchmarks/stage06p_memory/corpus/meldra_{strategy}/generated.c"
        )
...
def run_fair_memory_strategy_benchmark(
    *,
    output_dir: str | Path = "benchmarks/fair_memory_strategy",
    repetitions: int = 30,
    warmups: int = 5,
    ...
    Path("benchmarks/meldra_fair_memory_strategy.json").write_text(
        text, encoding="utf-8"
    )
    return report
