...
from .representation_mir import lower_rir_to_performance_mir, optimize_general_mir
from .structured_hir_v2 import compile_structured_hir_file
...
def _compile_arms(root: Path) -> dict[str, list[str] | None]:
    artifact = root / "benchmarks" / "general_representation"
    source_path = root / "merlo" / "programs" / "general_json.mlo"
...
def run_general_representation_benchmark(root: str | Path = ".") -> dict[str, Any]:
    ...
    surface = {
        ...
        "c": _surface_metrics(root_path / "benchmarks" / "general_representation" / "reference_json.c", "c"),
        "python": _surface_metrics(
            (
                root_path / "benchmarks" / "general_representation" / "reference_json.py",
