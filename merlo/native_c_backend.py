...
class CEmitter:
    ...
    def emit(self) -> str:
        lines = [
            ...
            "    if (length) { if (left.length) memcpy(data, left.data, (size_t)left.length); if (right.length) memcpy(data + left.length, right.data, (size_t)right.length); }",
            "    if (length) { ++meldra_heap_allocations; meldra_allocated_bytes += length; }",
