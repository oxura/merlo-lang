...
from .non_elidable_region import (
    DEFAULT_N,
    DEFAULT_SEED,
    RECORD_CAPACITY,
    TRAVERSAL_FACTOR,
...
REPETITIONS = 30
WARMUPS = 5
_OLD_FAIR = Path("benchmarks/meldra_fair_memory_strategy.json")
_OLD_NON_ELIDABLE = Path("benchmarks/meldra_non_elidable_region.json")
...
_FROZEN_ROOT = Path("benchmarks/non_elidable_region/corpus")
_ARM_ORDER = (
...
def run_constant_knowledge_audit(
    *,
    output_dir: str | Path = "benchmarks/constant_knowledge_audit",
    ...
    Path("benchmarks/meldra_constant_knowledge_audit.json").write_text(text, encoding="utf-8")
    return report


def refresh_constant_knowledge_analysis(
    report_path: str | Path = "benchmarks/meldra_constant_knowledge_audit.json",
) -> dict[str, Any]:
    """Refresh source-mapped loop analysis without rerunning saved timings."""
    path = Path(report_path)
    previous_bytes = path.read_bytes()
    report = json.loads(previous_bytes)
    root = Path("benchmarks/constant_knowledge_audit")
    loop_root = root / "hot_loops"
    source_paths = {
