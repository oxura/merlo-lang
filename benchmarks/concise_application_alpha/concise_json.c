...
static uint8_t *merlo_host_read_path(const char *path, uint64_t *length) {
    ...
    while (!feof(file)) {
        ...
        if (count == 0) break;
        if (used > SIZE_MAX - count) { free(data); fclose(file); return NULL; }
        ...
        if (required > capacity) {
            ...
            if (grown == NULL) { free(data); fclose(file); return NULL; }
            data = grown;
            capacity = next;
        }
...
int main(int argc, char **argv) {
    ...
    uint64_t length = 0;
    uint8_t *input = merlo_host_read_path(argv[1], &length);
    if (input == NULL) {
        fprintf(stderr, "AppError.ReadFailure: %s\n", argv[1]);
        return 74;
    }
    MerloBytesView view = { input, length };
    if (merlo_fn_box_smoke(UINT64_C(41)) != UINT64_C(41)) { free(input); return 70; }
    Merlo_ProgramResult result = merlo_fn_main(view);
    if (result.ok) {
    ...
    fprintf(stderr, "MERLO_METRICS allocations=%" PRIu64 " frees=%" PRIu64 " text_allocations=%" PRIu64 " text_frees=%" PRIu64
