...
CASES=[[{'operation': 'migrate', 'payload': {'name': 'Ada'}}, {'text': 'Hello, Ada!'}], [{'operation': 'migrate', 'payload': {'name': 'Adax'}}, {'text': 'Hello, Adax!'}], [{'operation': 'migrate', 'payload': {'name': 'Adax'}}, {'text': 'Hello, Adax!'}]]
UNTOUCHED=[({'operation': 'legacy', 'payload': {'name': 'Adax'}}, {'text': 'Hello Adax'})]
...
def check(workspace,arm):
 ...
 return {"case_id":'m07-rename-module',"passed":all(migration) and all(untouched_ok),"migration_passed":migration,"unaffected_passed":untouched_ok,"cases":len(CASES),"unaffected_cases":len(UNTOUCHED)}
