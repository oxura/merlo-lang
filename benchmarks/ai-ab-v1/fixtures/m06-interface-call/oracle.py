...
CASES=[[{'operation': 'migrate', 'payload': {'request_id': 'r9'}}, {'request_id': 'r9', 'status': 'ok'}], [{'operation': 'migrate', 'payload': {'request_id': 'r9x'}}, {'request_id': 'r9x', 'status': 'ok'}], [{'operation': 'migrate', 'payload': {'request_id': 'r9x'}}, {'request_id': 'r9x', 'status': 'ok'}]]
UNTOUCHED=[({'operation': 'legacy', 'payload': {'request_id': 'r9x'}}, {'status': 'ok', 'request_id': 'missing'})]
...
def check(workspace,arm):
 ...
 return {"case_id":'m06-interface-call',"passed":all(migration) and all(untouched_ok),"migration_passed":migration,"unaffected_passed":untouched_ok,"cases":len(CASES),"unaffected_cases":len(UNTOUCHED)}
