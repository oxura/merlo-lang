...
CASES=[[{'operation': 'migrate', 'payload': {'value': 5}}, {'seen': 5}], [{'operation': 'migrate', 'payload': {'value': 6}}, {'seen': 6}], [{'operation': 'migrate', 'payload': {'value': 6}}, {'seen': 6}]]
UNTOUCHED=[({'operation': 'legacy', 'payload': {'value': 6}}, {'seen': '6'})]
...
def check(workspace,arm):
 ...
 return {"case_id":'m09-callback-signature',"passed":all(migration) and all(untouched_ok),"migration_passed":migration,"unaffected_passed":untouched_ok,"cases":len(CASES),"unaffected_cases":len(UNTOUCHED)}
