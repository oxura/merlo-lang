...
CASES=[[{'operation': 'migrate', 'payload': {'timeout_ms': 250}}, {'timeout': 250}], [{'operation': 'migrate', 'payload': {'timeout_ms': 251}}, {'timeout': 251}], [{'operation': 'migrate', 'payload': {'timeout_ms': 251}}, {'timeout': 251}]]
UNTOUCHED=[({'operation': 'legacy', 'payload': {'timeout_ms': 251}}, {'timeout': 0})]
...
def check(workspace,arm):
 ...
 return {"case_id":'m05-config-key',"passed":all(migration) and all(untouched_ok),"migration_passed":migration,"unaffected_passed":untouched_ok,"cases":len(CASES),"unaffected_cases":len(UNTOUCHED)}
