...
| `Canonical contract 6`, HIR contract `13`, RIR contract `6`, MIR contract `3` | Production | Typed lowering is Merlo-owned; HIR JSON is the strict `merlo.structured-typed-hir.v13` contract with one closed TypeArena snapshot, digest, validated TypeIds, and separate deterministic machine-state identities. Transition nodes carry no type. Ownership, Place lookup, borrow provenance, structural ContractGraph schemes, RIR descriptors/drop plans, and MIR operands/results/places use TypeId authority under issues #84 and #85; RIR descriptors bind physical layouts through LayoutId. Generated-C semantics are unchanged; older HIR v12, RIR v5, and MIR v2 artifacts are rejected. Issue #86 covers user-defined generic declaration/package provenance; issue #72 covers native-syntax removal and executable-MIR backend authority. |
...
`0.1.0-alpha.3-dev` line with language `0.3`, frontend `8`, canonical `6`, HIR `13`,
RIR `6`, MIR `3`, runtime ABI `2`, and SemanticWorld `20`.
