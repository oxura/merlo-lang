...
from research.archive.alpha1.merlo.text_core_experiment import (
    _compile_raw,
    _compile_rust,
    _pinned_command,
    _rss_kb,
...
def _predecessor_cache_snapshot(root: Path) -> dict[str, str]:
    paths: list[Path] = []
    paths.extend(root.glob("tools/benchmarks/merlo/benchmarks/meldra_bytes*.json"))
    text_artifact = root / "tools/benchmarks/merlo/benchmarks/meldra_text_core_sprint.json"
    if text_artifact.is_file():
        paths.append(text_artifact)
    for directory in (
        root / "tools/benchmarks/merlo/benchmarks/meldra_text_core_sprint",
        root / "tools/benchmarks/merlo/benchmarks/meldra_bytes_builder",
    ):
        if directory.is_dir():
...
def run_text_builder_experiment(
    *,
    output_dir: str | Path = "tools/benchmarks/merlo/benchmarks/meldra_text_builder",
    report_path: str | Path = "tools/benchmarks/merlo/benchmarks/meldra_text_builder.json",
) -> dict[str, Any]:
