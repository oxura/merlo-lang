...
STAGE05P_FROZEN_PATHS = (
    ...
    "tools/benchmarks/merlo/benchmarks/meldra_stage04e_decision.json",
...
def build_stage05p_freeze(root: str | Path = ".") -> dict[str, Any]:
    ...
    decision = json.loads(
        (root_path / "tools" / "benchmarks" / "merlo" / "benchmarks" / "meldra_stage04e_decision.json").read_text(
            encoding="utf-8"
        )
    )
    return {
...
def verify_stage05p_freeze(root: str | Path = ".") -> Stage05PFreezeVerification:
    root_path = Path(root)
    path = root_path / "tools" / "benchmarks" / "merlo" / "benchmarks" / STAGE05P_FREEZE_FILENAME
    raw = path.read_bytes()
