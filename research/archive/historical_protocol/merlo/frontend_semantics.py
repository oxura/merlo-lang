...
from research.archive.historical_protocol.merlo.core_semantics import CoreProgram, CoreWorld, compile_core
...
class _Compiler:
    ...
    def _special_diagnostics(self) -> None:
    
        self._check_public_interface_drift()
        if self.diagnostics:
            return
        ...
        if self.diagnostics:
            return
        for cst in self.csts:
            for declaration in cst.module.declarations:
                if declaration.kind not in {"fn", "task"}:
                    continue
                if declaration.kind == "fn" and self._contains_effect_call(declaration.body):
                    self._error(
                        "PureEffectViolation",
                        "pure function performs an effectful operation",
                        cst.path,
                        declaration.span,
                    )
                    return
                if declaration.kind == "task":
                    ...
                    for effect in self._effectful_calls(declaration.body):
                        if effect not in declared and not any(
                            effect.startswith(item.rsplit(".", 1)[0] + ".")
                            for item in declared
                            if "." in item
                        ):
                            self._error(
                                "MissingCapability",
                                f"effect {effect!r} is not covered by the task capability environment",
                                cst.path,
                                declaration.span,
                            )
                            return
                if self._resource_not_closed(declaration.body):
                    self._error(
                        "ResourceNotClosed",
                        "owned resource is live on a return path",
                        cst.path,
                        declaration.span,
                    )
                    return
                if self._stale_line_view(declaration.body):
                    self._error(
                        "StaleLineView",
                        "borrowed line view is used after its reader advances",
                        cst.path,
                        declaration.span,
                    )
                    return

    def _check_public_interface_drift(self) -> None:
        for cst in self.csts:
            exported = set(cst.module.exports)
            signatures: dict[str, tuple[tuple[str, ...], str | None]] = {}
            for declaration in cst.module.declarations:
                if declaration.name not in exported:
                    continue
                signature = (
                    tuple(parameter.type_name for parameter in declaration.parameters),
                    declaration.return_type,
                )
                previous = signatures.get(declaration.name)
                if previous is not None and previous != signature:
                    self._error(
                        "PublicInterfaceDrift",
                        f"exported declaration {declaration.name!r} has incompatible signatures",
                        cst.path,
                        declaration.span,
                    )
                    return
    ...
    def _check_import_graph(self) -> None:
        modules = {
            (cst.module.package_name, cst.module.module_name): cst
            for cst in self.csts
        }
        graph: dict[tuple[str, str], list[tuple[str, str]]] = {}
        for key, cst in modules.items():
            targets: list[tuple[str, str]] = []
            for use in cst.module.uses:
                if "internal" in use.source.split(".") or "private" in use.source.split("."):
                    self._error(
                        "PrivateModuleAccess",
                        f"private module {use.source!r} cannot be imported",
                        cst.path,
                        use.span,
                    )
                    return
                parts = use.source.split(".")
                target = (parts[0], ".".join(parts[1:])) if len(parts) > 1 else (use.source, "")
        ...
        def visit(node: tuple[str, str], path: tuple[tuple[str, str], ...]) -> bool:
            if node in visiting:
                cycle_nodes = (
                ...
                cycle = " -> ".join(
                    f"{package}.{module}"
                    for package, module in cycle_nodes
                )
                code = (
                    "PackageCycle"
                    if len({package for package, _module in cycle_nodes}) > 1
                    else "CyclicImport"
                )
                cst = modules[node]
                self._error(
                    code,
                    f"cyclic module dependency: {cycle}",
                    cst.path,
                    cst.module.span,
                )
                return True
    ...
    def _qualified_expression(self, expression: Expression | None) -> str | None:
        if expression is None:
            return None
        if expression.kind == "name":
            return expression.name
        if expression.kind == "field" and expression.children:
            parent = self._qualified_expression(expression.children[0])
            return f"{parent}.{expression.name}" if parent else None
        if expression.kind == "call" and expression.children:
            return self._qualified_expression(expression.children[0])
        if expression.kind == "try" and expression.children:
            return self._qualified_expression(expression.children[0])
        return None
    def _iter_expressions(self, statements: tuple[Statement, ...]) -> Iterable[Expression]:
        for statement in statements:
            if statement.expression is not None:
                yield from self._iter_expression_nodes(statement.expression)
            yield from self._iter_expressions(statement.body)
            yield from self._iter_expressions(statement.else_body)
            for arm in statement.arms:
                yield from self._iter_expression_nodes(arm.expression)

    def _effectful_calls(self, statements: tuple[Statement, ...]) -> set[str]:
        effects: set[str] = set()
        for expression in self._iter_expressions(statements):
            if expression.kind != "call":
                continue
            name = self._qualified_expression(expression)
            if name and name.startswith(("fs.", "console.")):
                effects.add(name)
        return effects

    def _contains_effect_call(self, statements: tuple[Statement, ...]) -> bool:
        return bool(self._effectful_calls(statements))

    def _resource_not_closed(self, statements: tuple[Statement, ...]) -> bool:
        def walk(items: tuple[Statement, ...], live: set[str]) -> tuple[set[str], bool]:
            current = set(live)
            for statement in items:
                if statement.kind == "let" and statement.name and statement.expression is not None:
                    if self._qualified_expression(statement.expression) == "fs.open":
                        current.add(statement.name)
                expression_name = self._qualified_expression(statement.expression)
                if expression_name and expression_name.endswith(".close"):
                    receiver = expression_name.rsplit(".", 1)[0]
                    current.discard(receiver)
                if statement.kind == "return":
                    return current, bool(current)
                if statement.kind == "if":
                    _, then_bad = walk(statement.body, current)
                    _, else_bad = walk(statement.else_body, current) if statement.else_body else (set(current), False)
                    if then_bad or else_bad:
                        return current, True
                if statement.kind not in {"if", "return"}:
                    continue
            return current, False

        _, bad = walk(statements, set())
        return bad

    def _stale_line_view(self, statements: tuple[Statement, ...]) -> bool:
        borrowed: dict[str, str] = {}
        advanced: set[str] = set()

        def visit(items: tuple[Statement, ...]) -> bool:
            for statement in items:
                expression = statement.expression
                if statement.kind == "let" and statement.name and expression is not None:
                    assigned = self._qualified_expression(expression)
                    if assigned and assigned.endswith(".next_line"):
                        borrowed[statement.name] = assigned.rsplit(".", 1)[0]
                if expression is not None:
                    for nested in self._iter_expression_nodes(expression):
                        if nested.kind != "call":
                            continue
                        name = self._qualified_expression(nested)
                        if not name:
                            continue
                        if name.endswith(".next_line"):
                            advanced.add(name.rsplit(".", 1)[0])
                        if name.endswith(".to_text") and nested.children:
                            field = nested.children[0]
                            receiver = (
                                self._qualified_expression(field.children[0])
                                if field.kind == "field" and field.children
                                else self._qualified_expression(field)
                            )
                            if receiver in borrowed and borrowed[receiver] in advanced:
                                return True
                if statement.kind == "if" and (
                    visit(statement.body) or visit(statement.else_body)
                ):
                    return True
            return False
    ...
    def _iter_expression_nodes(self, expression: Expression) -> Iterable[Expression]:
        yield expression
        for child in expression.children:
            yield from self._iter_expression_nodes(child)
        for argument in expression.arguments:
            yield from self._iter_expression_nodes(argument.expression)
