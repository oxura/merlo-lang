...
from research.archive.alpha1.merlo.native_hypotheses import NATIVE_HYPOTHESES_FILENAME
from research.archive.historical_protocol.merlo.stage05p_freeze import STAGE05P_FREEZE_FILENAME, assert_stage05p_frozen
from research.archive.historical_protocol.merlo.stage05p_protocol import STAGE05P_DECISIONS, STAGE05P_PROTOCOL_FILENAME
...
def build_stage05p_decision(root: str | Path = ".") -> dict[str, Any]:
    ...
    benchmark_path = root_path / "tools" / "benchmarks" / "merlo" / "benchmarks" / "stage05p_runs" / "report.json"
    hypotheses_path = root_path / "tools" / "benchmarks" / "merlo" / "benchmarks" / NATIVE_HYPOTHESES_FILENAME
    protocol_path = root_path / "tools" / "benchmarks" / "merlo" / "benchmarks" / STAGE05P_PROTOCOL_FILENAME
    ...
    return {
        ...
        "inputs": {
            ...
            for path in (
                root_path / "tools" / "benchmarks" / "merlo" / "benchmarks" / STAGE05P_FREEZE_FILENAME,
                protocol_path,
                benchmark_path,
                hypotheses_path,
            )
...
def write_stage05p_decision(root: str | Path = ".") -> dict[str, Any]:
    ...
    (root_path / "tools" / "benchmarks" / "merlo" / "benchmarks" / STAGE05P_DECISION_FILENAME).write_text(
