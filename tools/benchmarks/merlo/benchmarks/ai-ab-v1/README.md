...
The draft locks the alpha2 compiler baseline (commit/tag, language grammar,
edition, package version) and carries six disjoint unrun calibration tasks,
excluded from final metrics. External provider, image, and root-commit locks
remain explicit blockers.

The preregistration component root is:

```text
protocol_sha256 = 87924b5b10ba02c2cd45775205e2f6d93379a9e3f5d7bfa7a1eea81aad9ac416
tasks_sha256    = a387364b22e1b8f77527e376df2979a92c317bec9e57bbcdcf06765a7dc7ba31
combined_root   = 94c1380c9a6020a05c1b789a4fa9f38de95e09740537ec2fb250d3c9527538a3
```
...
anchor. The alpha2 baseline and six calibration manifests are bound; the
external root remains explicitly `AWAITING_PARENT_PREREGISTRATION_COMMIT_SHA`,
and provider/container locks remain unbound.
...
salt; the API key itself is never published.
```console
python3 -m tools.benchmarks.merlo.ai_ab --root tools/benchmarks/merlo --provider-config LOCKED_PROVIDER_CONFIG --output ai-ab-v1-report.json
```

This draft has no provider execution command enabled; running the validator
cannot synthesize provider identity or outcomes.
