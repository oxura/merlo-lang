...
from tools.benchmarks.merlo.productive_simplicity import (
    APPLICATIONS,
    ARMS,
...
def _write_arm(root: Path, application: str, arm: str, source: str) -> None:
    if arm == "concise_merlo":
        path = root / "tools" / "benchmarks" / "merlo" / "programs" / f"productive_{application}" / "app" / "main.mlo"
    elif arm == "canonical_merlo":
        path = root / "tools" / "benchmarks" / "merlo" / "programs" / f"productive_{application}" / "canonical" / "main.mlo"
    else:
        suffix = {"python": ".py", "c": ".c"}[arm]
        path = root / "tools" / "benchmarks" / "merlo" / "benchmarks" / "productive_simplicity" / application / f"reference{suffix}"
...
def test_audit_measures_every_merlo_module(tmp_path: Path) -> None:
    _write_arm(tmp_path, "ndjson", "concise_merlo", "fn main():\n    return 0\n")
    module = tmp_path / "tools/benchmarks/merlo/programs/productive_ndjson/app/report.mlo"
    ...
    assert concise["source_files"] == [
        "tools/benchmarks/merlo/programs/productive_ndjson/app/main.mlo",
        "tools/benchmarks/merlo/programs/productive_ndjson/app/report.mlo",
    ]
