...
LOCKED_PROTOCOL_SHA256 = "87924b5b10ba02c2cd45775205e2f6d93379a9e3f5d7bfa7a1eea81aad9ac416"
LOCKED_TASKS_SHA256 = "a387364b22e1b8f77527e376df2979a92c317bec9e57bbcdcf06765a7dc7ba31"
PREREGISTRATION_ROOT_SHA256 = "94c1380c9a6020a05c1b789a4fa9f38de95e09740537ec2fb250d3c9527538a3"
...
def validate_protocol(root: str | Path | None = None) -> Validation:
    ...
    if protocol.get("baseline") != {
        "commit_sha": "23f0516c645c592b057497d0e55030e81e18f5ba",
        "tag": "v0.1.0-alpha.2",
        "language_version": "0.2",
        "grammar_version": "0.2",
        "edition": "alpha.1",
        "compiler_version": "0.1.0-alpha.2",
        "package_version": "0.1.0-alpha.2",
    }:
        _error(errors, "alpha2 compiler baseline changed")
    ...
    try:
        calibration = _load_json(study / "calibration" / "calibration.json")
    except ProtocolError as exc:
        return Validation(False, tuple(errors + [str(exc)]), len(ids), len(pair_ids), {"tasks": len(ids), "pairs": len(pair_ids), "arm_attempts": len(pair_ids) * 2})
    calibration_items = calibration.get("tasks")
    if (
        calibration.get("schema_version") != "merlo.ai-ab.v1.calibration"
        or calibration.get("status") != "UNRUN"
        or calibration.get("task_count") != 6
        or calibration.get("excluded_from_final_metrics") is not True
        or calibration.get("overlap_with_final_tasks") != 0
        or not isinstance(calibration_items, list)
        or len(calibration_items) != 6
        or calibration.get("tasks_sha256") is not None
        or protocol.get("calibration_policy", {}).get("manifest_sha256") != sha256_file(study / "calibration" / "calibration.json")
    ):
        _error(errors, "calibration manifest lock changed")
    calibration_ids: set[str] = set()
    final_oracles = {
        str(task.get("oracle", {}).get("path"))
        for task in task_items
        if isinstance(task, dict)
    }
    for calibration_task in (calibration_items if isinstance(calibration_items, list) else ()):
        calibration_id = calibration_task.get("id") if isinstance(calibration_task, dict) else None
        if not isinstance(calibration_id, str) or calibration_id in calibration_ids or calibration_id in ids:
            _error(errors, f"calibration task overlaps or duplicates final task: {calibration_id!r}")
            continue
        calibration_ids.add(calibration_id)
        if calibration_task.get("excluded_from_final_metrics") is not True or "outcome" in calibration_task:
            _error(errors, f"calibration task is not excluded/unrun: {calibration_id}")
        if calibration_task.get("tool_menu") != protocol.get("tool_menu") or calibration_task.get("budgets") != protocol.get("budgets") or calibration_task.get("provider_identity") != protocol.get("provider_identity"):
            _error(errors, f"calibration contract mismatch: {calibration_id}")
        oracle = calibration_task.get("oracle", {})
        oracle_path = str(oracle.get("path", ""))
        if oracle_path in final_oracles:
            _error(errors, f"calibration oracle overlaps final oracle: {calibration_id}")
        oracle_file = study / oracle_path
        if not oracle_file.is_file() or oracle.get("sha256") != sha256_file(oracle_file):
            _error(errors, f"calibration oracle hash mismatch: {calibration_id}")
        for arm in ("merlo", "python"):
            fixture = calibration_task.get("fixtures", {}).get(arm, {})
            fixture_root = study / str(fixture.get("path", ""))
            if not fixture_root.is_dir() or fixture.get("sha256") != tree_digest(fixture_root):
                _error(errors, f"calibration fixture hash mismatch: {calibration_id}/{arm}")
    if len(calibration_ids) != 6:
        _error(errors, "expected six disjoint calibration tasks")
