...
from merlo.native_differential import HIREvaluator, MIRInterpreter
...
def test_decision_artifact_is_internally_valid() -> None:
    report = json.loads(
        Path("benchmarks/meldra_bytes_reborrow.json").read_text(
            encoding="utf-8"
        )
    )
