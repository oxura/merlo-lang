...
from merlo.productive_milestone import (
...
class ProductiveMilestoneTests(unittest.TestCase):
    ...
    def test_supported_requires_all_phase_gates(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            (root / "benchmarks/merlo_concise_application_alpha.json").write_text("alpha", encoding="utf-8")
            (root / "benchmarks/merlo_general_representation_core.json").write_text("general", encoding="utf-8")
    ...
    def test_validator_rejects_stale_predecessor(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            root = Path(directory)
            (root / "benchmarks").mkdir()
            alpha = root / "benchmarks/merlo_concise_application_alpha.json"
            general = root / "benchmarks/merlo_general_representation_core.json"
