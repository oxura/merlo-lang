...
from merlo.general_representation_corpus import (
    invalid_json_cases,
    layout_sources,
    valid_json_cases,
)
from merlo.general_representation_falsification import run_falsification_controls
from merlo.general_representation_milestone import (
    SUPPORTED,
...
SOURCE_PATH = Path("merlo/programs/general_json.mlo")
...
def test_final_milestone_artifact_is_self_consistent_and_supported():
    report = json.loads(
        Path("benchmarks/merlo_general_representation_core.json").read_text(
            encoding="utf-8"
        )
    )
